...
img=cv2.imread('12.jpg') # change for any picture
  ...
  for cnt in contours:
    ...
    if area > areamin:
      ...
      cv2.putText(imgContour, "Points: " + str(len(approx)), (x, y + 20), cv2.FONT_HERSHEY_COMPLEX, .5,
                        (0, 255, 0), 1)
      cv2.putText(imgContour, "Area: " + str(int(area)), (x, y + 40), cv2.FONT_HERSHEY_COMPLEX, 0.5,
                        (0, 255, 0), 1)
      cv2.putText(imgContour, "Center: ("+ str(x+int(w/2)) +","+ str(y+int(h/2))+ ")", (x, y + 80), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 1)
