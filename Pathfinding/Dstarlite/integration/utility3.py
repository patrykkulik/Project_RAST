...
def draw_grid3(graph, ax, view_range, real_world_multiplier, real_world_multiplierZ, orientation, **style):
    ...
    if orientation == 0:
        xs = xs[int(0.25*end):int(0.75*end)]
        ys = ys[int(0.25*end):int(0.75*end)]
    elif orientation == 45:
        ...
        xs = xs[:int(end*0.5-0.073375*end)]
        ys = ys[:int(end*0.5-0.073375*end)]
    elif orientation == 180:
        xs = xs[:int(end*0.25)]
        ys = ys[:int(end*0.25)]
    elif orientation == -45:
        xs = xs[int(end*0.5-0.073375*end):int(0.75*end+0.073375*end)]
        ys = ys[int(end*0.5-0.073375*end):int(0.75*end+0.073375*end)]
    elif orientation == -90:
        xs = xs[int(0.5*end):]
        ys = ys[int(0.5*end):]
    elif orientation == -135:
...
def grid_from_string3(string, length, width, height):
    ...
    g = grids3.SquareGrid3(length, width, height)  # Create a Grid that is equivalent to the input grid
    start, end = None, None
    ...
    for layer in range(len(layers)):
        ...
        for row, line in enumerate(lines):  # Get the current row and the current line of the grid
            for col, char in enumerate(line):  # Get the current column and the current character of the grid
                ...
                if char == grids3.WALL:  # If the character is the same as a defined wall character,
...
def initDomain(length, width, height, start):
    """
    Construct a SquareGrid3 from LxWxH
    Args:
        :type length: int
        :type width: int
        :type height: int
    ...
    g = grids3.SquareGrid3(length, width, height)  # Create a Grid that is equivalent to the input grid
    return g, start, None  # return the grid object, start and end
