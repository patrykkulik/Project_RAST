...
from utility3 import draw_grid3, label_grid3, grid_from_string3
from priority_queue import PriorityQueue
from grids3 import AgentViewGrid3, SquareGrid3
...
class adaptive3(object):
    ...
    def initialise_plot(self):
        ...
        # ax_label = plt.add_subplot(1, 2, 2)
        ax_label = 1
        ax = 1
    ...
    def adapt_grid(self, new_walls, ax, robot_plot=True):
        ...
        for wall in new_walls:
            ...
            # ax.plot([self.real_world_multiplier * wall[0]], [self.real_world_multiplier * wall[1]], [self.real_world_multiplierZ * wall[2]], 'ks')
        ...
        for new_node in new_nodes:
            for neighbor in self.graph.neighbors_adapt(new_node):
                if distance3(new_node, neighbor) >= 0.25 and np.round(neighbor[2] * 2) / 2 == self.position[2] and np.round(new_node[2] * 2) / 2 == self.position[2]:
    ...
    def initialise(self):
        ...
        walls = self.graph.new_walls(observation)  # Define the position of newly observed walls to the graph seen by the robot
        ...
        # draw_grid3(self.graph, ax, self.view_range, self.real_world_multiplier, self.real_world_multiplierZ,self.orientation,path=self.position, goal=self.goal)

        return ax  # WON'T NEED TO HAVE THIS IN THE REAL CODE

    def move_to_goal(self, ax):
        # print("move_to_goal(): self.real_graph.slamData {}".format(len(self.real_graph.slamData)))
        ...
        print("move_to_goal(): observation {}".format(len(observation)))
        
        to_remove = []
        for a in observation:
            if not self.graph.in_bounds(a):
                to_remove.append(a)

        # NOTE: Uncomment these lines if you wish to print the processed SLAM data
        for a in observation:
            if observation[a] == '#':
        ...
        # print("move_to_goal(): new_walls {}".format(len(new_walls)))
        ...
        if new_walls:
            ...
            if self.position == self.start:
                ...
                print("available nodes {}".format(len(self.graph.available_nodes)))
                print("seen nodes {}".format(len(self.seen)))
...
def main():
    ...
    while dstar.position != dstar.start or len(dstar.seen) != len(dstar.graph.available_nodes):
        ...
        waypoint = dstar.move_to_goal(ax)
