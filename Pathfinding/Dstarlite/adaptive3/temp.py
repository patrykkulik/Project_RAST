...
class SquareGrid3:
    ...
    def observe(self, position, orientation, obs_range=2):
        ...
        for a in self.available_nodes:
            if distance2(a, position) <= obs_range and distanceZ(a, position) <= 1 and orientation - 90 <= angle(
                    position, a) <= orientation + 90:
                a = tuple(np.round(a, 4))
                ret_dict[a] = PASSABLE
        for w in self.walls:
            if distance2(w, position) <= obs_range and distanceZ(a, position) <= 1 and orientation - 90 <= angle(
                    position, w) <= orientation + 90:
