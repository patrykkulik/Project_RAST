...
class FieldDStarLite(object):
    def __init__(self, graph, start, goal, view_range=1.7):
        ...
        self.G_VALS = {start: float('inf'),
                       goal: float('inf')}  # Dictionary of all g values for all of the expanded cells
        self.RHS_VALS = {start: float('inf'), goal: 0}  # Dictionary of all rhs values for all of the expanded cells
    ...
    def compute_cost(self, s,  sa, sb, diagonals):
    ...
    def calculate_rhs(self, node):
        ...
        if self.obstacle_check(node):  # If node is in an obstacle, rhs is equal to inf
            ...
            for pair in connbrs:  # For every pair in the connbrs list
                ...
                vs_temp, destination = self.compute_cost(node, pair[0], pair[1], diagonals)  # compute the cost
